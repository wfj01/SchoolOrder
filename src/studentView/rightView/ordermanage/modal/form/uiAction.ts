...
export class FromViewUiAction{
     ...
     * 表单数据
     */
    private formData:OrderManagementEntity;
    ...
    private validateServer(values:any){
        ...
        const {GlobalOrderManagementviewDoMainStore}=this.props;

        if (!GlobalOrderManagementviewDoMainStore) {
            this.isValidated = true;
            return;
        }

        const otherError = GlobalOrderManagementviewDoMainStore.ValiDate(values);
        if (otherError) {
