...
export class CollegeTownDoMainStore {
    /**
     * 开始号码
     */
    @observable
     ...
     */
    @observable
    public selectRowListData: CollegeTownEntity[];
     ...
     */
    @observable
    public currentEditCardType:CollegeTownEntity;
    ...
    constructor() {
        ...
        this.List = new Array<CollegeTownEntity>();
    }
     ...
     * @param index 
     */
    public getRowIndex(record: CollegeTownEntity): string {
        return record.id;
    }
    /**
     * 加载数据
    ...
    public async LoadData() {
        try {
            this.isLoading = true;
            const res = await requestJson("/api/CollegeTown/queryUser",
                {
                    method: "GET"
                })
                if (res.rtnCode !== 0) {
    ...
    public async SaveBtn(): Promise<IResponseJsonResult> {
        try {
            ...
            const res:any = await requestJson("/api/CollegeTown/postUser",
                {
                    method: "POST",
     ...
     */
    @action
    private recursionSelect(itemId:string,CardTypeList:CollegeTownEntity[]){
