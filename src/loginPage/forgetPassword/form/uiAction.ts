...
export class ForgetPassWordFormUiAction{
    ...
    public handleForgetPassWord(event: React.ChangeEvent<HTMLInputElement>){
        this.props.GlobalListViewDoMainStore!.ForgetPassWord = event.target.value;
    }

    public handleConfirmPassword(event: React.ChangeEvent<HTMLInputElement>){
        this.props.GlobalListViewDoMainStore!.ConfirmPassword = event.target.value;
    }
    ...
    private validateServer(values:any){
        ...
        const {GlobalLoginPageStore}=this.props;

        if (!GlobalLoginPageStore) {
            this.isValidated = true;
            return;
        }

        const otherError = GlobalLoginPageStore.validate(values);
        if (otherError) {
