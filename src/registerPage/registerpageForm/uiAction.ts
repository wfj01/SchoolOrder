...
export class RegisterPageFormUiAction{
     ...
     * 表单数据
     */
    private formData:RegisterPageFormEntity;


    /**
     * 当前表单的api接口
     */
    private props:IRegisterPageProps;
    ...
    public handlePassWord(event: React.ChangeEvent<HTMLInputElement>){
        this.props.GlobalListViewDoMainStore!.handlePassWord = event.target.value;
    }

    public handleConfirmPassword(event: React.ChangeEvent<HTMLInputElement>){
        this.props.GlobalListViewDoMainStore!.handleConfirmPassword = event.target.value;
    }
    ...
    private validateServer(values:any){
        ...
        const {GlobalRegisterPageDomainStore}=this.props;

        if (!GlobalRegisterPageDomainStore) {
            this.isValidated = true;
            return;
        }

        const otherError = GlobalRegisterPageDomainStore.validate(values);
        if (otherError) {
