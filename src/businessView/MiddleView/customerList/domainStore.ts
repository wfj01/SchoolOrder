...
import { CustomerListEntity } from "./entity";

export class CustomerListDomainStores{

    @observable
    public isLoading:boolean;

    @observable
    public allReportTableData:CustomerListEntity[];

    @observable
    public showReportTableData:CustomerListEntity[];
    ...
    constructor(){
        ...
        this.PageIndex = 1
        this.PageSize = 20;
        this.showReportTableData = new Array<CustomerListEntity>();
    }
    ...
    public async Loaddata(){
        try {
            this.isLoading = true;
            const res = await requestJson("/api/CustomerList/queryUser",
                {
                    method: "GET"
                })
                if (res.rtnCode !== 0) {
            ...
            this.aaa = data;
            this.aaa.map((element:CustomerListEntity)=>{
                if(element.sex === true){
                    element.sextext = "男"
                }
                else
