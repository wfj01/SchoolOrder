...
import { RegisterPageViewEntity } from "../../entity";
import { IRegisterPageViewProps } from "./interface";

export class RegisterPageViewUiAction{

    /**
     * 定义接口
     */
    private props:IRegisterPageViewProps;

    
    /**
     * 表单数据
     */
    private formData:RegisterPageViewEntity;
    ...
    public handlePassWord(event: React.ChangeEvent<HTMLInputElement>){
        this.props.GlobalBusinessListViewDoMainStore!.handlePassWord = event.target.value;
    }

    public handleConfirmPassword(event: React.ChangeEvent<HTMLInputElement>){
        this.props.GlobalBusinessListViewDoMainStore!.handleConfirmPassword = event.target.value;
    }
